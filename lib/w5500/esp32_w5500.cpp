                        ...
                        uint8_t *W5500_Mac)
{
  ...
  {
    log_e("SPI Clock must be >= 8 and <= 25 MHz for W5500");
...
bool ESP32_W5500::config(IPAddress local_ip, IPAddress gateway, IPAddress subnet, IPAddress dns1, IPAddress dns2)
{
  ...
  {
    log_e("DHCP could not be stopped! Error = %s", err);
    return false;
  }
  ...
  if (err != ERR_OK)
  {
    log_e("STA IP could not be configured! Error = %s", err);
    return false;
  }
  ...
  else
  {
    ...
    {
      log_i("DHCP could not be started! Error = %s", err);
      return false;
    }
